...
```
`users`, `activities`, `dashboard`, and `setup` use a narrower shape without the `data`/`errors` wrapper — this is original, intentional, per-route API behavior; do not "fix" it into the standard envelope without confirming the frontend still expects the old shape (it does). `reminders/whatsapp` was rewritten to use the standard envelope when the Send Message page was added (see below) — nothing previously called it, so there was no old shape to preserve.
...
- `GET /api/reminders/whatsapp?window=15|10|5|expired` / `POST /api/reminders/whatsapp` — powers the Send Message page (`/dashboard/messages`). `GET` lists active policies for one of four expiry checkpoints (exact-day offsets 15/10/5, or expired within the last 15 days — all computed from `new Date()` at request time, never a fixed date); `POST` takes a caller-resolved `{ recipients: [{ policyId, phone, message }] }` array and actually calls the WhatsApp Cloud API (`src/lib/whatsapp.ts`, using `WHATSAPP_TOKEN`/`WHATSAPP_PHONE_NUMBER_ID`/optional `WHATSAPP_API_VERSION`) — no longer a simulation. Message text is resolved client-side from the customizable template in Settings (`pf_whatsapp_settings` in `localStorage`) before being sent to this route. Note: Meta's Cloud API only allows freeform `text` messages within a 24-hour customer-initiated session window; cold outreach like expiry reminders will need an approved Message Template registered in Meta Business Manager to be reliable at scale — `sendWhatsAppTextMessage` sends `type: "text"` today and would need a `template` payload to support that.
