...
Every entity except `ActivityLog` has `isActive`, and `Vehicle` still soft-deletes on its own (deactivation sets it to `false` rather than removing the row, blocked at the service layer if it has an active `Policy`). `Customer` and `Policy` are the exception: their Delete action is a **permanent hard delete** (`CustomerRepository.delete`/`PolicyRepository.delete`, real `DELETE FROM`), not a soft delete — this was changed deliberately from the original all-soft-delete design, so don't "fix" it back. `Policy` has no child records, so deleting one has no guard beyond existence.

Deleting a `Customer` (`customerService.deleteCustomer`) is a cascading, atomic operation, not a plain single-row delete:
1. Runs inside one `transaction()` (`src/lib/database.ts`) — if any step fails, everything rolls back and nothing is deleted.
2. First checks the customer's active `Policy` count. If **any** are active, the whole operation aborts immediately with a friendly error ("This customer has an active policy. Please cancel or complete the policy before deleting the customer.") and nothing is written — this is the one condition that blocks deletion at all.
3. If that check passes, it explicitly deletes in dependency order: all of the customer's `Policy` rows (only inactive/expired/cancelled ones can exist at this point), then all of their `Vehicle` rows, then the `Customer` row itself.

Because child rows are deleted explicitly before the parent, a foreign-key conflict can never occur here — there is deliberately **no** `P2003` catch in this flow (unlike `DELETE /api/users/[id]`, which still relies on the FK constraint since Users aren't cascade-deleted). If you ever see a `P2003` surface from `deleteCustomer`, the explicit delete-order above has been broken and needs fixing, not re-wrapping in another try/catch. Uniqueness (vehicle number, engine number, chassis number, policy number) is enforced at the database level via Prisma `@unique` **globally** (not scoped to `isActive` — a deactivated vehicle permanently reserves its numbers), while phone-number uniqueness for `Customer` is intentionally **not** a DB constraint — it's enforced only in `customerService.ts` and only among active customers.
