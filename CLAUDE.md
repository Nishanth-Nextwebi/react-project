...
```
`users`, `activities`, `dashboard`, `reminders/whatsapp`, and `setup` use a narrower shape without the `data`/`errors` wrapper — this is original, intentional, per-route API behavior; do not "fix" it into the standard envelope without confirming the frontend still expects the old shape (it does).
